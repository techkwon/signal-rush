  ...
  return (
    <main className={`signal-game ${motionReduced ? "reduced" : ""}`} style={{ "--accent": stage.accent, "--world": stage.color } as React.CSSProperties}
      ...
      {screen === "playing" && <section className={`live-runner energy-${stageIndex + 1} route-${route} ${gameEffect} ${boostActive ? "overdrive" : ""} ${stageIndex === 0 ? `tutorial-step-${prologueGuideIndex}` : ""}`}>
        ...
        <div className={`challenge-hud ${stageIndex === 0 ? "tutorial-hud" : ""}`} aria-live={stageIndex === 0 ? "polite" : undefined}><span>{stageIndex === 0 ? "PROLOGUE" : `STAGE ${stageIndex + 1}`} · WAVE {courseIndex + 1}/{courseTotal} · {isRoute ? "ROUTE CHOICE" : challenge.kicker}</span><strong>{challenge.prompt}</strong><small>{stageIndex === 0 ? `${prologueGuide.eyebrow} · ${prologueGuide.title}` : `${dataPaces[stageIndex]} · 큰 데이터는 보낼 조각이 많아 더 오래 달립니다`}</small><div className="approach-bar"><i style={{ width: `${progress}%` }} /></div><div className="stage-wave">{Array.from({ length: courseTotal }, (_, index) => <i key={index} className={index <= courseIndex ? "on" : ""} />)}</div></div>
